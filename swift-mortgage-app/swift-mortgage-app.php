                    ...
                    if ($fLog) :
                        foreach ($fLog as $log) :
                            ...
                            <tr id='user-<?php echo $log->id; ?>' class="alternate">
                                ...
                                <td class="filename column-filename"><?php
                                    if ($log->filename)
                                        echo $log->filename;
                                    else
                                        echo '-';
                                ...
                                <td class="email column-email">
                                    <?php if ($log->email) { ?><a href='mailto:<?php echo $log->email; ?>' title='E-mail: <?php echo $log->email; ?>'><?php echo $log->email; ?></a><?php
                                    } else {
                                        echo '-';
                                    }
                                    ?>
                                ...
                                <td class="email column-email">
                                    <?php if ($log->phone) { ?><a href='tel:<?php echo $log->phone; ?>' title='Phone: <?php echo $log->phone; ?>'><?php echo $log->phone; ?></a><?php
                                    } else {
                                        echo '-';
                                    }
                                    ?>
...
function sma_admin_dispplay_log_settings() {
    ...
    <div class="wrap">
        ...
        <?php
        if (
                isset($_POST['save_sma_form']) && wp_verify_nonce($_POST['save_sma'], 'save_sma')
        ) {
...
function sma_load_scripts() {
    ...
    wp_enqueue_script('swift-leadpage-js', plugins_url('/js/leadpage.js', __FILE__), array('jquery'), '', true);
    ...
    if (isset($_COOKIE['sma_log_id'])) {
        ...
        $wpdb->update(
            ...
            'daet_time' => date('Y-m-d h:i:s'),
                ), array('id' => $_COOKIE['sma_log_id']), array(
            '%s', // value1
            '%s' // value2
                ), array('%d')
        );
...
add_action('wp_ajax_sma_leadpage', 'sma_leadpage');
add_action('wp_ajax_nopriv_sma_leadpage', 'sma_leadpage');